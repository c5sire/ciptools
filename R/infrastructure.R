# This fragment is from Jim Heesters pull request https://github.com/jimhester/devtools/blob/
# 65041349864cae5205036572afd9ee0f7b8a0272/R/infrastructure.R
library(devtools)
library(whisker)
library(stringr)
...
use_coveralls <- function(pkg = ".") {
  ...
  if(!any(grep("sudo: required", travis_content))) {
    travis_content <- append("sudo: required\n", travis_content)
  }
  
  if (any(grep("library(covr);coveralls()", travis_content))) {
  ...
  if(!any(grep( "covr", travis_content))) {
